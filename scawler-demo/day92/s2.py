...
"authenticity_token": authenticity_token,
    "utf8": "",
    "commit": "Sign in",
    "login": "3dimenrobot@gmail.com",  # 3dimenrobot@gmail.com
    'password': 'Android*0'   # Android*0
}
...
print(soup3,list_group.children)

from bs4.element import Tag

for child in list_group.children:
    if isinstance(child, Tag):
        project_tag = child.find(name='a', class_='ml-2')
        size_tag = child.find(name='span',class_="text-small")
        print(project_tag,size_tag)
        temp =  ("项目:%s(%s); 项目路径:%s" % (project_tag.get('href'), size_tag.string, project_tag.string, ) )  if  project_tag else ""
        print(temp)
